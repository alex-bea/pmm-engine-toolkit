...
Use this sole RUN for normal editorial execution. Detailed installation, mapping, fixture,
receipt, and repair guidance lives in `REF-pre-read-sharpener-setup-contract.md` and is loaded
only through the conditional route in `../SKILL.md`.

## Output-route preflight

- `inline`: begin immediately. No setup configuration or receipt is required, and no local
  artifact is written.
- persistent local output: begin only after the router confirms that the adopter-owned
  receipt is `ready` for the current package, setup contract, configuration, required
  mappings, and dependency set.
- if persistence is requested and status is `missing`, `stale`, or `blocked`, stop this RUN
  and follow the setup contract. Do not invent a destination or repair setup here.

A ready persistent route must not reread setup detail, rerun the fictional fixture, or
rewrite configuration or receipt state.
...
## Step 5 — Persist when the selected route is ready

For an `inline` route, skip this step and do not write a file.

For a ready persistent route, write the complete Step 3 and Step 4 result beneath the
configured output destination using:

```text
outputs/pre-reads/YYYY-MM-DD-<slug>-pre-read.md
```

The path is never inside the installed skill package. Create the configured output directory
when absent and authorized.
...
If the configured workspace becomes unwritable after routing, return the passing inline
deliverable and report the persistence failure. Do not silently choose another destination
or modify setup state.

## Step 6 — Return

For a persistent write, put the written path on the first line. Then return the same complete
content inline. For an inline route, return the complete content without a path or
...
5. update the same dated file only when the original route was persistent and ready, unless
   the user explicitly requests a new version; and
...
| Inline route selected | Return the complete result without writing setup state or a deliverable file. |
| Persistent status is `missing`, `stale`, or `blocked` | Stop and route to the setup contract; do not choose another destination. |
| Output directory is absent on a ready route | Create it in the configured authorized workspace. |
| Initial output path exists | Use the next numeric suffix; do not overwrite. |
| Configured workspace becomes unwritable | Return inline and report the persistence failure; do not write elsewhere. |
