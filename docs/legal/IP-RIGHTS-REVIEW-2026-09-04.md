# IP Rights Review — Govern Skills 0.3.0
...
documentation text is copied into the candidate. Public action references retain their
existing repository-reviewed pins and notices.

## Fictional example

The Cedar Harbor Software scenario is independently invented, explicitly fictional, and
uses reserved `.invalid` domains. Its organization, reviewer, revision, source, artifact,
and adapter values are test-only. It is not an anonymized private run and supplies no real
approval or publication authority.

## Disposition

The scoped artifacts are suitable for inclusion under the repository's Apache-2.0 license,
subject to the full test, sanitization, secrets, diff-boundary, and maintainer review gates.
The generated `docs/legal/IP-INVENTORY.csv` remains the per-file disposition record.
