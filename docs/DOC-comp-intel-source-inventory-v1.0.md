...
The inventory remains advisory design history. The shipped
[competitive-intelligence starter kit](../skills/comp-intel/README.md) is the current product
direction. The earlier [Codex product-requirements suite](product-requirements/comp-intel/README.md)
is also retained as non-binding design history and does not override the package.
...
| `skills/comp-intel/examples/EX-synthetic.md` | Fictional walkthrough; public baseline | Synthetic | Same path | `rewrite-generic` | Same path plus `examples/fictional-embedded-wallets/` and separate controller fixtures | Expand the safe example into a full-depth, non-reversible structural mirror; never publish real entities, facts, or a one-to-one alias map. | CI-CFG-003, CI-PRV-002, CI-QA-001 | Template-parity, referential-integrity, chronology, traceability, and privacy tests |
| `skills/comp-intel/references/RUN-workflow.md`; `references/RUN-onboarding.md` | Recurring workflow and guided first-run setup | Public-safe | Partial workflow only | `rewrite-generic` | Same paths | Package the complete review-gated analyst method plus a guided source-verification and positioning-approval path for a first baseline; keep the controller as optional advanced support. | CI-PKG-003, CI-CFG-001–003, CI-RUN-001–005 | Document audit; onboarding and workflow tests |
| `skills/comp-intel/README.md`; `references/REF-analyst-contract.md` | Installation, document map and analyst behavior | Public-safe | None | `rewrite-generic` | Same paths | Make the package understandable and executable by Claude Code, Codex, or another compatible agent without private context. | CI-PKG-001–004, CI-EVD-001–003, CI-OUT-001 | Skill validation; framework tests |
| `skills/comp-intel/assets/{onboarding-state,source-map,adopter-positioning,competitor-registry,positioning-context,evidence-log,run-record,tracker}-template*.md` | Fillable setup and operating structures | Public-safe placeholders | Partial config and schema assets | `replace-with-template-or-schema` | Same paths | Give adopters the human-readable structures used by the golden workflow, including a resumable setup record and a separately approved adopter position, while retaining structured schemas as optional support. | CI-CFG-001–003, CI-RUN-001–004, CI-STATE-001–002 | Package closure; onboarding-gate tests; fictional example parity |
...
| Source | Role / direction | Sensitivity | Current public equivalent | Disposition | Target artifact | Rationale | Requirements | Verification |
|---|---|---|---|---|---|---|---|---|
| `docs/STD-lexicon-consumer-contract-v1.0.md` | Private terminology policy; inbound | Organization-specific | None | `interface-only` | Adopter-owned market pack and positioning context | Allow adopter-provided terminology rules without a repository-root runtime dependency. | CI-PKG-003, CI-CFG-001, CI-PRV-001 | Direct-install closure test |
...
| Source | Role / direction | Sensitivity | Current public equivalent | Disposition | Target artifact | Rationale | Requirements | Verification |
|---|---|---|---|---|---|---|---|---|
| `outputs/competitive/*-signals.md` | Collected evidence cache; output/input on resume | Potentially private and untrusted | None | `replace-with-template-or-schema` | `assets/evidence-log-template.md`; `assets/schemas/evidence-record.schema.json`; run manifest | Normalize evidence before review and make the set resumable; advanced mode binds synthesis to its digest. | CI-EVD-001–003, CI-RUN-001–003 | Evidence and resume tests |
...
| `outputs/competitive/battlecard-gaps.md` | Shared gap tracker; bidirectional | Private strategy | None | `replace-with-template-or-schema` | `assets/tracker-templates.md`; `assets/schemas/change-set.schema.json` | Changes remain proposed until separately reviewed and applied. | CI-RUN-004, CI-STATE-002 | Concurrency and apply tests |
| `outputs/competitive/narrative-tracker.md` | Narrative history; bidirectional | Private strategy | None | `replace-with-template-or-schema` | `assets/tracker-templates.md`; claim and registry schemas | Preserve dated history without subjective in-place mutation during collection. | CI-EVD-001–003, CI-STATE-002 | Narrative-proof test |
| `outputs/competitive/win-loss-log.md` | Deal-signal tracker; bidirectional | Critical customer data | None | `replace-with-template-or-schema` | `assets/tracker-templates.md` under adopter data root | Never bundle rows; require explicit sensitivity and confirmation state. | CI-PRV-001–003, CI-STATE-002 | Sensitivity propagation test |
| `state/runs/comp-intel-*.yaml` | Resume and completion state; bidirectional pattern | Internal run metadata | None | `replace-with-template-or-schema` | `assets/run-record-template.md`; `assets/schemas/run-state.schema.json` | Preserve scope, stage history, resume context, and optional digest-bound review state. | CI-RUN-002–004, CI-STATE-001 | Same-day, resume and corrupt-state tests |
...
| Source | Role / direction | Sensitivity | Current public equivalent | Disposition | Target artifact | Rationale | Requirements | Verification |
|---|---|---|---|---|---|---|---|---|
| `docs/workbench/{master requirements, implementation blueprint, acceptance plan, four implementation PRDs}` | Prior private design inputs; analysis only | Mixed private context and obsolete decisions | Archived public Codex product-requirements suite | `retain-private` | Non-binding design history under `docs/product-requirements/comp-intel/` | Keep inaccessible private files out of the runtime package; the shipped framework is self-contained. | CI-MIG-002, CI-PRV-001, CI-QA-002 | Public-link and package-content scans |

## 9. Closure conclusion

The functional closure contains:

- the original public package plus the complete agent-neutral workflow, analyst contract,
  fillable operating templates, and fictional worked example;
