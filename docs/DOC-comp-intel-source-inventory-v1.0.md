...
| `skills/comp-intel/references/RUN-workflow.md` | Workflow summary; public baseline | Public-safe but incomplete | Same path | `rewrite-generic` | Same path | Replace the controller-only sequence with the complete review-gated analyst method and keep the controller as optional advanced support. | CI-PKG-003, CI-RUN-001–005 | Document audit; workflow tests |
...
| `skills/comp-intel/assets/{source-map,competitor-registry,evidence-log,run-record,tracker}-template*.md` | Fillable operating structures | Public-safe placeholders | Partial config and schema assets | `replace-with-template-or-schema` | Same paths | Give adopters the actual human-readable structures used by the golden workflow while retaining structured schemas as optional support. | CI-CFG-001–003, CI-RUN-001–004, CI-STATE-001–002 | Package closure; fictional example parity |
