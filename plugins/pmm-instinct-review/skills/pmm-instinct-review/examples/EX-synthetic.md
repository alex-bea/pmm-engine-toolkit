...
- bounded, redacted, conversation-only Claude evidence;
- digest-keyed queue completion and one schema-valid voice suggestion;
- an explicit cluster decision and approved instinct;
- an exact local promotion preview, immutable receipt, and verified outcome;
- an alternative governed-review patch that does not mutate its target; and
- final isolated runtime status plus before/after `CLAUDE.md` content.

Bundled examples are inert documentation. They are sequential snapshots, not a live state
directory, and the runtime never loads them by default.
